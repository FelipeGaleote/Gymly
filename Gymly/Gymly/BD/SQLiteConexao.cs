    ...
    class SQLiteConexao
    {
        ...
        private void CriaTabelas()
        {
            if (!File.Exists(nomeBanco))
            {
                ...
                sql.AppendLine("[AVALIADOR] VARCHAR(70),");
                sql.AppendLine("[DATA] DATE,");
                sql.AppendLine("[TIPO] VARCHAR(30),");
                sql.AppendLine("[OBSERVACAO]  VARCHAR(1024),");
                sql.AppendLine("[CAMINHOIMAGEMFRONTAL]  VARCHAR(1024),");
                sql.AppendLine("[OBSERVACAOIMAGEMFRONTAL]  VARCHAR(1024),");
                sql.AppendLine("[CAMINHOIMAGEMLATERAL]  VARCHAR(1024),");
                sql.AppendLine("[OBSERVACAOIMAGEMLATERAL]  VARCHAR(1024),");
                sql.AppendLine("[CAMINHOIMAGEMCOSTAS]  VARCHAR(1024),");
                sql.AppendLine("[OBSERVACAOIMAGEMCOSTAS] VARCHAR(1024),");
               ...
               sql.AppendLine("[ENVERGADURA] FLOAT,");
                ...
                //AVALIADORES
                sql.AppendLine("CREATE TABLE IF NOT EXISTS AVALIADORES ([NOME] VARCHAR(70) PRIMARY KEY);");
                cmd = new SQLiteCommand(sql.ToString(), conn);
                cmd.ExecuteNonQuery();
                conn.Close();
            }
